...
<?= \Codemoi\Core\Csrf::field() ?>
        ...
        <div class="w-full md:w-auto flex flex-col sm:flex-row gap-3">
            <div>
                <label class="block text-xs font-medium text-ink-500 mb-1 uppercase tracking-wider">Từ ngày</label>
                <input type="date" name="from_date" value="<?= isset($from_date) ? htmlspecialchars($from_date) : '' ?>"
                    class="w-full px-4 py-2 border border-ink-300 rounded-lg focus:ring-2 focus:ring-brand-500 transition-colors bg-ink-50 text-ink-700">
            </div>
            <div>
                <label class="block text-xs font-medium text-ink-500 mb-1 uppercase tracking-wider">Đến ngày</label>
                <input type="date" name="to_date" value="<?= isset($to_date) ? htmlspecialchars($to_date) : '' ?>"
                    class="w-full px-4 py-2 border border-ink-300 rounded-lg focus:ring-2 focus:ring-brand-500 transition-colors bg-ink-50 text-ink-700">
            </div>
            <div>
                <label class="block text-xs font-medium text-ink-500 mb-1 uppercase tracking-wider">Sắp xếp SP bán
                    ra</label>
            ...
            <div class="flex items-end">
                <button type="submit"
                    class="w-full px-6 py-2.5 bg-brand-600 text-white font-medium rounded-lg hover:bg-brand-700 focus:ring-4 focus:ring-brand-300 transition-all cursor-pointer shadow-sm">
                    Lọc dữ liệu
                </button>
            </div>
        </div>
    ...
    <div class="bg-ink-800/70 backdrop-blur-xl rounded-xl shadow-sm border border-ink-200 overflow-hidden flex flex-col">
        ...
        <div class="p-0 flex-1 overflow-auto max-h-[400px]">
            <table class="w-full text-left border-collapse text-sm">
                <thead class="sticky top-0 bg-ink-50 z-10 shadow-sm">
                    <tr class="text-ink-600 text-xs font-semibold uppercase tracking-wide">
                        <th class="px-6 py-3 border-b border-ink-200">Ngày</th>
                        <th class="px-6 py-3 border-b border-ink-200 text-center">Số đơn hàng</th>
                        <th class="px-6 py-3 border-b border-ink-200 text-right">Doanh thu</th>
                    </tr>
                </thead>
        ...
        <?php if (!empty($revenue_stats)): ?>
            <div class="px-6 py-3 bg-ink-50 border-t border-ink-200 flex justify-between items-center">
    ...
    <div class="bg-ink-800/70 backdrop-blur-xl rounded-xl shadow-sm border border-ink-200 overflow-hidden flex flex-col">
        ...
        <div class="p-0 flex-1 overflow-auto max-h-[400px]">
            <table class="w-full text-left border-collapse text-sm">
                <thead class="sticky top-0 bg-ink-50 z-10 shadow-sm">
                    <tr class="text-ink-600 text-xs font-semibold uppercase tracking-wide">
                        <th class="px-6 py-3 border-b border-ink-200">Sản phẩm</th>
                        <th class="px-6 py-3 border-b border-ink-200 text-center">Đã bán</th>
                        <th class="px-6 py-3 border-b border-ink-200 text-right">Doanh thu mang lại</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-ink-100">
                        ...
                        <tr class="hover:bg-ink-50 transition-colors">
                            <td class="px-6 py-3 flex items-center gap-3">
                                <img src="./uploads/<?= e($stat['img_pro']) ?>"
                                    class="w-10 h-10 rounded-md border border-ink-200 object-cover">
    ...
    <div class="p-0 overflow-auto max-h-[500px]">
        <table class="w-full text-left border-collapse text-sm">
            <thead class="sticky top-0 bg-ink-50 z-10 shadow-sm">
                <tr class="text-ink-600 text-xs font-semibold uppercase tracking-wide">
                    <th class="px-6 py-3 border-b border-ink-200">Sản phẩm</th>
                    <th class="px-6 py-3 border-b border-ink-200">Đơn giá</th>
                    <th class="px-6 py-3 border-b border-ink-200 text-center">Tồn kho hiện tại</th>
                    <th class="px-6 py-3 border-b border-ink-200 text-center">Tình trạng</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-ink-100">
