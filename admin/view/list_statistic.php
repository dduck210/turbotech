        ...
        <div class="w-full md:w-auto flex flex-col sm:flex-row gap-3">
            <div>
                ...
                <input type="date" name="from_date" value="<?= isset($from_date) ? htmlspecialchars($from_date) : '' ?>"
                    class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-brand-500 transition-colors bg-slate-50 text-slate-700">
            </div>
            <div>
            ...
            <div>
                <label class="block text-xs font-medium text-slate-500 mb-1 uppercase tracking-wider">Sắp xếp SP bán
                    ra</label>
...
<div class="grid grid-cols-1 xl:grid-cols-2 gap-6 mb-8">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden flex flex-col">
        <div class="px-6 py-4 border-b border-slate-200 bg-slate-50/50 flex justify-between items-center">
            <h2 class="font-semibold text-slate-800"><i class="fas fa-chart-line text-emerald-500 mr-2"></i>Doanh thu
                theo ngày</h2>
        </div>
            ...
            <table class="w-full text-left border-collapse text-sm">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    ?>
                        <tr class="hover:bg-slate-50 transition-colors">
                            ...
                            <td class="px-6 py-3 text-center"><span
                                    class="bg-emerald-100 text-emerald-700 px-2.5 py-0.5 rounded-full font-medium"><?= $stat['total_orders'] ?></span>
                            </td>
                    ...
                    <?php if (empty($revenue_stats)): ?>
                        <tr>
                            <td colspan="3" class="px-6 py-8 text-center text-slate-500">Không có dữ liệu doanh thu trong
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden flex flex-col">
        <div class="px-6 py-4 border-b border-slate-200 bg-slate-50/50 flex justify-between items-center">
            <h2 class="font-semibold text-slate-800"><i class="fas fa-fire text-amber-500 mr-2"></i>Sản phẩm bán chạy
            </h2>
        </div>
        <div class="p-0 flex-1 overflow-auto max-h-[400px]">
            <table class="w-full text-left border-collapse text-sm">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <?php if (empty($product_sold_stats)): ?>
                        <tr>
                            <td colspan="3" class="px-6 py-8 text-center text-slate-500">Không có dữ liệu sản phẩm bán ra.
                            </td>
                        </tr>
                    <?php endif; ?>
                </tbody>
...
<div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden mb-8">
    <div class="px-6 py-4 border-b border-slate-200 bg-slate-50/50 flex justify-between items-center">
        <h2 class="font-semibold text-slate-800"><i class="fas fa-boxes text-blue-500 mr-2"></i>Tình trạng tồn kho (Tất
            cả sản phẩm)</h2>
