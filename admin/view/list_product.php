...
<?php /** @var array $ds_loai */ /** @var array $listsp */ ?>

<div class="mb-8 flex items-center justify-between">
    <h1 class="text-3xl font-bold text-slate-800">Quản Lý Sản Phẩm</h1>
    <a href="index.php?act=add_product" class="px-5 py-2.5 bg-brand-600 text-white font-medium rounded-lg hover:bg-brand-700 focus:ring-4 focus:ring-blue-300 transition-all flex items-center gap-2">
    ...
    <div class="p-6">
        <div class="mb-6 bg-slate-50 p-4 rounded-lg border border-slate-100 flex items-center justify-between">
            ...
            <form action="./index.php?act=list_product" method="POST" class="flex gap-3">
                <select name="idcate" class="px-4 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-brand-500 focus:border-brand-500 transition-colors bg-white min-w-[200px]">
            ...
            <table class="w-full text-left border-collapse" id="example">
                <thead>
                    <tr class="bg-slate-50 border-y border-slate-100 text-slate-600 text-sm font-semibold tracking-wide uppercase">
                        ...
                        <th class="px-4 py-4 text-center"><i class="far fa-eye text-slate-400" title="Lượt xem"></i></th>
                ...
                <tbody class="divide-y divide-slate-100">
                    <?php foreach ($listpro as $pro) : ?>
                        <tr class="hover:bg-slate-50 transition-colors">
                            ...
                            <td class="px-4 py-4">
                                <div class="w-14 h-14 rounded-lg border border-slate-200 overflow-hidden bg-white flex items-center justify-center">
                                    <img src="./uploads/<?= $pro['img_pro'] ?>" alt="<?= htmlspecialchars($pro['name_pro']) ?>" class="max-w-full max-h-full object-cover">
                                </div>
                            </td>
                            <td class="px-4 py-4">
                                <div class="font-medium text-slate-800 line-clamp-2"><?= $pro['name_pro'] ?></div>
                                <div class="text-xs text-slate-500 mt-1 line-clamp-1" title="<?= htmlspecialchars($pro['short_des']) ?>"><?= $pro['short_des'] ?></div>
                            </td>
                            ...
                            <td class="px-4 py-4 text-center">
                                <?php if ($pro['discount'] > 0): ?>
                                    <span class="inline-flex items-center justify-center px-2.5 py-1 text-xs font-bold rounded-full bg-red-100 text-red-600">
                                        -<?= $pro['discount'] ?>%
                                    </span>
                                <?php else: ?>
                            ...
                            <td class="px-4 py-4 text-center">
                                <?php if ((int) $pro['stock'] <= 0) : ?>
                                    <span class="inline-flex items-center justify-center px-2.5 py-1 border border-red-200 text-xs font-medium rounded-full bg-red-50 text-red-600">
                                        Hết hàng
                                    </span>
                                <?php else : ?>
                                    <span class="inline-flex items-center justify-center px-2.5 py-1 border border-green-200 text-xs font-medium rounded-full bg-green-50 text-green-700">
                                        <?= $pro['stock'] ?>
                                    </span>
                                <?php endif; ?>
                            </td>
                            ...
                            <td class="px-4 py-4 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    ...
                                    <a href="./index.php?act=delete_product&id_pro=<?= $pro['id_pro'] ?>" class="p-2 text-red-600 bg-red-50 rounded-lg hover:bg-red-100 transition-all active:scale-90" data-confirm="Bạn có chắc chắn muốn xóa không?" title="Xóa">
