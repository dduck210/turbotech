  ...
  <div class="p-6">
    <div class="form-addcate">
      <form action="index.php?act=update_user" method="post" data-validate novalidate>
        ...
        ?>
        <div class="mb-4">
          <label for="formGroupExampleInput" class="block text-sm font-medium text-slate-700 mb-1">Mã người
            dùng</label>
          <input type="text" name="id_user"
            ...
            placeholder="Mã KH(auto increase)" value="<?= $id_user ?>" disabled>
        </div>
        <div class="mb-4">
          <label for="formGroupExampleInput" class="block text-sm font-medium text-slate-700 mb-1">Tên đăng
            nhập</label>
          <input type="text" name="user_name" data-rules="required|min:3|max:50"
            class="w-full rounded-lg border border-slate-200 px-4 py-2 focus:ring-2 focus:ring-brand-500 outline-none transition-all"
            placeholder="Nhập tên dùng để đăng nhập" value="<?= $user_name ?>">
        </div>
        <div class="mb-4">
          <label for="formGroupExampleInput" class="block text-sm font-medium text-slate-700 mb-1">Họ
            tên</label>
          <input type="text" name="full_name" data-rules="required|min:2|max:100"
            class="w-full rounded-lg border border-slate-200 px-4 py-2 focus:ring-2 focus:ring-brand-500 outline-none transition-all"
            placeholder="Nhập họ và tên người dùng" value="<?= $full_name ?>">
        </div>
        <div class="mb-4">
          ...
          <input type="email" name="email_user" data-rules="required|email"
            class="w-full rounded-lg border border-slate-200 px-4 py-2 focus:ring-2 focus:ring-brand-500 outline-none transition-all"
            placeholder="Nhập email người dùng" value="<?= $email_user ?>">
        </div>
        <div class="mb-4">
        ...
        <div class="mb-4">
          <label for="" class="block text-sm font-medium text-slate-700 mb-1">Vai trò: <span
              style="color:red">
              <?php if ($role == 1) {
                echo "Admin";
              } else {
