...
<div class="mb-8 pb-5 border-b border-ink-300">
    ...
    <h1 class="font-heading text-3xl text-ink-900">Hóa Đơn</h1>
</div>

<div class="card-boutique rounded-lg overflow-hidden">
    <div class="p-6">
        <div class="mb-6 pb-6 border-b border-ink-200">
            <div class="flex items-center gap-2 text-ink-700 font-semibold mb-4">
...
<?= \Codemoi\Core\Csrf::field() ?>
                ...
                <div class="w-full lg:flex-1">
                    <label class="block text-xs font-semibold text-ink-500 mb-1.5 uppercase tracking-wider">Từ khóa
                        tìm kiếm</label>
                    <input type="text" name="keyword"
                        value="<?= isset($keyword) ? htmlspecialchars($keyword) : '' ?>"
                        placeholder="Tên khách hàng, SĐT..."
                        class="w-full px-4 py-2 border border-ink-300 rounded-md focus:ring-2 focus:ring-brand-400 focus:border-brand-500 transition-colors bg-ink-50 text-ink-800">
                </div>

                <!-- Status -->
                <div class="w-full lg:w-48">
                    <label class="block text-xs font-semibold text-ink-500 mb-1.5 uppercase tracking-wider">Trạng
                        thái</label>
                    <select name="status"
                        class="w-full px-4 py-2 border border-ink-300 rounded-md focus:ring-2 focus:ring-brand-400 focus:border-brand-500 transition-colors bg-ink-50 text-ink-800">
                ...
                <div class="w-full lg:w-auto flex flex-col sm:flex-row gap-3">
                    <div>
                        <label class="block text-xs font-semibold text-ink-500 mb-1.5 uppercase tracking-wider">Từ
                            ngày</label>
                        <input type="date" name="from_date"
                            value="<?= isset($from_date) ? htmlspecialchars($from_date) : '' ?>"
                            class="w-full px-4 py-2 border border-ink-300 rounded-md focus:ring-2 focus:ring-brand-400 focus:border-brand-500 transition-colors bg-ink-50 text-ink-700">
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-ink-500 mb-1.5 uppercase tracking-wider">Đến
                            ngày</label>
                ...
                <div class="w-full lg:w-auto">
                    <button type="submit" name="btn_filter"
                        class="w-full px-6 py-2 border border-ink-300 text-ink-700 font-medium rounded-md hover:bg-ink-200 transition-colors cursor-pointer whitespace-nowrap">
            ...
            <table class="w-full text-left border-collapse" id="table1">
                <thead>
                    <tr class="border-b-2 border-ink-300 text-ink-600 text-xs font-semibold tracking-widest uppercase">
                        ...
                        <th scope="col" class="px-4 py-3 text-center">Trạng thái thanh toán</th>
                        <th scope="col" class="px-4 py-3 text-center">Trạng thái đơn hàng</th>
                    ...
                    ?>
                        <tr class="hover:bg-ink-100/60 transition-colors">
                            ...
                            <td class="px-4 py-4 text-center"><?php if ($bill['status_pay'] == 0) {
                                                                    echo "<span class='inline-flex items-center justify-center px-2.5 py-1 text-xs font-bold rounded-md bg-amber-500/15 text-amber-700'>Chưa thanh toán</span>";
                                                                } else if ($bill['status_pay'] == 1) {
                                                                    echo "<span class='inline-flex items-center justify-center px-2.5 py-1 text-xs font-bold rounded-md bg-emerald-500/15 text-emerald-700'>Đã thanh toán</span>";
                                                                } else {
                                                                    echo "Không tìm thấy phương thức thanh toán";
                                                                }  ?></td>
                            <td class="px-4 py-4 text-center"><?php if ($bill['status'] == 0) {
                                                                    echo "<span class='inline-flex items-center justify-center px-2.5 py-1 text-xs font-bold rounded-md bg-cyan-100 text-cyan-700'>Đơn hàng mới</span>";
                                                                } else if ($bill['status'] == 1) {
                                                                    echo "<span class='inline-flex items-center justify-center px-2.5 py-1 text-xs font-bold rounded-md bg-amber-500/15 text-amber-700'>Đang xử lý</span>";
                                                                } else if ($bill['status'] == 2) {
                                                                    echo "<span class='inline-flex items-center justify-center px-2.5 py-1 text-xs font-bold rounded-md bg-brand-500/15 text-brand-700'>Đang giao hàng</span>";
                                                                } else if ($bill['status'] == 3) {
                                                                    echo "<span class='inline-flex items-center justify-center px-2.5 py-1 text-xs font-bold rounded-md bg-emerald-500/15 text-emerald-700'>Đã giao hàng</span>";
                                                                } elseif ($bill['status'] == 4) {
                                                                    echo "<span class='inline-flex items-center justify-center px-2.5 py-1 text-xs font-bold rounded-md bg-red-500/15 text-red-600'>Đã hủy</span>";
                                                                } else {
                                                                    echo "Lỗi trạng thái";
                                                                } ?></td>
                                ...
                                <div class="flex items-center justify-center gap-2">
                                    ...
                                    <?php elseif ($bill['status'] == 1): ?>
                                        <a href="index.php?act=ship_bill&idbill=<?= e($bill['id_bill']) ?>&_token=<?= urlencode(\Codemoi\Core\Csrf::token()) ?>"
                                            class="p-2 text-indigo-600 bg-indigo-500/10 rounded-md hover:bg-indigo-500/15 transition-all active:scale-90"
                                    ...
                                    <a href="index.php?act=edit_bill&idbill=<?= e($bill['id_bill']) ?>"
                                        class="p-2 text-amber-600 bg-amber-500/10 rounded-md hover:bg-amber-500/15 transition-all active:scale-90"
                                        title="Cập nhật chi tiết"><i class="fas fa-edit"></i></a>
                                    <a href="index.php?act=billdetail&idbill=<?= e($bill['id_bill']) ?>"
                                        class="p-2 text-emerald-600 bg-emerald-500/10 rounded-md hover:bg-emerald-500/15 transition-all active:scale-90"
