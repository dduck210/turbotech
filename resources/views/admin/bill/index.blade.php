...
@section('content')
<h1 class="mb-6 font-heading text-2xl font-semibold text-ink-900">Đơn hàng</h1>
...
<form action="{{ route('admin.orders.index') }}" method="get" class="mb-6 flex flex-wrap gap-3 rounded-md border border-ink-300 bg-white p-4">
    <input type="text" name="keyword" value="{{ $keyword }}" placeholder="Tên khách, SĐT..." class="rounded-md border border-ink-300 px-3 py-2 text-sm">
    <select name="status" class="rounded-md border border-ink-300 px-3 py-2 text-sm">
        <option value="-1" @selected($status === -1)>Tất cả</option>
        @foreach ($statusLabels as $value => $label)
            <option value="{{ $value }}" @selected($status === $value)>{{ $label }}</option>
        @endforeach
    </select>
    <input type="date" name="from_date" value="{{ $fromDate }}" class="rounded-md border border-ink-300 px-3 py-2 text-sm">
    <input type="date" name="to_date" value="{{ $toDate }}" class="rounded-md border border-ink-300 px-3 py-2 text-sm">
    <button type="submit" class="rounded-md border border-ink-300 px-4 py-2 text-sm font-medium">Lọc</button>
</form>
        ...
        <tbody class="divide-y divide-ink-200">
            @foreach ($orders as $order)
                <tr>
                    ...
                    <td class="p-4 font-semibold text-brand-600">{{ number_format($order->total_amount) }}₫</td>
                    <td class="p-4">{{ $statusLabels[$order->status] ?? 'Đơn hàng mới' }}</td>
