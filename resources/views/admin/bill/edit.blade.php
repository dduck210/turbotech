...
@section('page-title', 'Đơn hàng #'.$order->bill_code)

@section('content')
<div class="max-w-lg rounded-md border border-ink-300 bg-white p-6 shadow-sm">
    ...
    <p class="mb-4 text-sm"><strong>Tổng tiền:</strong> <span class="price font-semibold">{{ number_format($order->total_amount) }}₫</span></p>
    ...
    <form action="{{ route('admin.orders.update', $order->id_bill) }}" method="post" class="space-y-4">
        ...
        <div>
            ...
            <select name="status_pay" class="input-boutique">
