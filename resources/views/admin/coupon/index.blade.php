...
<div class="mb-6 flex items-center justify-between">
    <h1 class="font-heading text-2xl font-semibold text-ink-900">Mã giảm giá</h1>
    <a href="{{ route('admin.coupons.create') }}" class="btn-boutique rounded-md px-4 py-2 text-sm font-semibold">Thêm mã</a>
</div>
        ...
        <tbody class="divide-y divide-ink-200">
            @foreach ($coupons as $coupon)
                <tr>
                    ...
                    <td class="p-4">{{ (int) $coupon->status === 1 ? 'Đang hoạt động' : 'Tắt' }}</td>
                    <td class="p-4">
