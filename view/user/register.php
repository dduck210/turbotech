...
<!-- Form đăng ký: split panel (brand quote + form), was: single centered glass card -->
        ...
        <div class="mx-auto grid max-w-4xl grid-cols-1 overflow-hidden rounded-lg border border-ink-300 lg:grid-cols-2">
            <div class="hidden flex-col justify-center bg-ink-900 p-10 text-ink-100 lg:flex">
                ...
                <p class="mt-4 font-heading text-2xl font-medium leading-snug text-ink-50">
                    &ldquo;Gia nhập cộng đồng khách hàng tin dùng Turbotech.&rdquo;
                </p>
                <span class="mt-6 block h-px w-12 bg-brand-500"></span>
                <p class="mt-6 text-sm text-ink-400">Tạo tài khoản để đặt hàng nhanh hơn, theo dõi đơn hàng và nhận ưu đãi dành riêng cho thành viên.</p>
            </div>
...
<?= \Codemoi\Core\Csrf::field() ?>
                    <h1 class="mb-6 font-heading text-2xl font-semibold text-ink-900">Đăng ký tài khoản</h1>

                    <?php if (isset($duplicateField) && in_array($duplicateField, ['phone', 'email'], true)) : ?>
                        <div class="mb-4 rounded-md border border-amber-500/30 bg-amber-500/10 p-3.5 text-sm text-amber-800">
