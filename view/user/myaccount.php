...
<main class="page-content bg-ink-100">
    ...
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10 md:py-14">
        <div class="grid grid-cols-1 gap-10 lg:grid-cols-4">
            ...
            <!-- Sidebar nav: thin left-accent active item (matches the admin sidebar recipe) -->
            <div class="lg:col-span-1">
                <nav aria-label="Điều hướng tài khoản" class="border border-ink-200 bg-ink-50">
                    <ul class="divide-y divide-ink-200" id="account-page-tab" role="tablist">
                        <li>
                            <a href="#account-dashboard" data-tab-target="account-dashboard" role="tab" aria-selected="true"
                                class="account-tab-trigger flex items-center gap-3 border-l-2 border-brand-500 bg-brand-50 px-4 py-3.5 text-sm font-semibold text-brand-700 focus:outline-none focus:ring-2 focus:ring-brand-500 focus:ring-inset">
                        ...
                        <li>
                            <a href="#account-orders" data-tab-target="account-orders" role="tab" aria-selected="false"
                                class="account-tab-trigger flex items-center gap-3 border-l-2 border-transparent px-4 py-3.5 text-sm font-semibold text-ink-700 hover:bg-ink-100 focus:outline-none focus:ring-2 focus:ring-brand-500 focus:ring-inset">
                        ...
                        <li>
                            <a href="index.php?act=logout" role="tab" aria-selected="false"
                                ...
                                class="flex items-center gap-3 border-l-2 border-transparent px-4 py-3.5 text-sm font-semibold text-red-600 hover:bg-red-500/10 focus:outline-none focus:ring-2 focus:ring-brand-500 focus:ring-inset">
            ...
            <div class="lg:col-span-3">
                <div id="account-page-tab-content">
                    ...
                    <div id="account-dashboard" data-tab-panel role="tabpanel" aria-labelledby="account-dashboard-tab"
                        class="account-tab-panel card-boutique p-6">
                        <h2 class="font-heading text-lg font-semibold text-ink-900 mb-4">Bảng điều khiển</h2>
                    ...
                    <div id="account-orders" data-tab-panel role="tabpanel" aria-labelledby="account-orders-tab"
                        class="account-tab-panel hidden card-boutique p-6">
                        <h2 class="mb-4 font-heading text-lg font-semibold text-ink-900">Đơn hàng của tôi</h2>
                        <?php if (!empty($cancelMessage)) : ?>
                            <div class="mb-4 rounded-md border border-ink-200 bg-ink-100 p-3 text-sm text-ink-700"><?= htmlspecialchars($cancelMessage) ?></div>
                        <?php endif; ?>
                        <?php if (empty($list_mybill)) : ?>
                            <div class="py-16 text-center">
                                <i class="fa-solid fa-box-open text-5xl text-ink-400" aria-hidden="true"></i>
                                                ...
                                                <td class="px-4 py-3">
                                                    <?php if ((int) $status === 0) : ?>
                                                        <form action="index.php?act=cancelorder" method="post"
                                                            ...
                                                            <button type="submit"
                                                                class="inline-flex items-center justify-center gap-1.5 rounded-md border border-red-500/30 px-3 py-1.5 text-xs font-semibold text-red-600 hover:bg-red-500/10 focus:outline-none focus:ring-2 focus:ring-brand-500">
                    ...
                    <div id="account-address" data-tab-panel role="tabpanel" aria-labelledby="account-address-tab"
                        class="account-tab-panel hidden card-boutique p-6">
                        <h2 class="font-heading text-lg font-semibold text-ink-900 mb-4">Địa chỉ</h2>
                    ...
                    <div id="account-details" data-tab-panel role="tabpanel" aria-labelledby="account-details-tab"
                        class="account-tab-panel hidden card-boutique p-6">
                        <h2 class="font-heading text-lg font-semibold text-ink-900 mb-4">Thông tin tài khoản</h2>
                        <form action="index.php?act=myaccount" method="post" enctype="multipart/form-data" data-validate novalidate>
<?= \Codemoi\Core\Csrf::field() ?>
                            <div class="mb-6 flex items-center gap-4 border-b border-ink-200 pb-6">
                                <img src="uploads/<?= e($_SESSION['user']['img_user']) ?>" alt="Avatar người dùng"
                                    class="h-20 w-20 rounded-full border border-ink-300 object-cover">
                                <div class="flex-1">
                                    ...
                                    <input type="file" name="img_user" id="account-details-avatar"
                                        class="block w-full text-sm text-ink-500 file:mr-4 file:rounded-md file:border-0 file:bg-brand-50 file:px-4 file:py-2 file:text-sm file:font-semibold file:text-brand-700 hover:file:bg-brand-100">
                                </div>
                            </div>
                            ...
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <input type="text" name="user_name" id="account-details-username" value="<?= e($_SESSION['user']['user_name']) ?>"
                                        ...
                                        class="block w-full rounded-md border border-ink-200 bg-ink-100 px-3.5 py-2.5 text-sm text-ink-500">
                                </div>
                                ...
                                <div>
                                    ...
                                    <select name="sex" id="account-details-sex" required
                                        class="block w-full rounded-md border border-ink-300 bg-white px-3.5 py-2.5 text-sm text-ink-900 focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-brand-500">
                                ...
                                <div>
                                    ...
                                    <select name="ward" id="account-details-ward" data-address-ward
                                        ...
                                        class="block w-full rounded-md border border-ink-300 bg-white px-3.5 py-2.5 text-sm text-ink-900 focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-brand-500" disabled>
                                ...
                                <div class="pt-2">
                                    ...
                                    <button type="submit" name="btn_change" value="Lưu thay đổi"
                                        class="btn-boutique inline-flex items-center justify-center gap-2 rounded-md px-5 py-2.5 text-sm font-semibold disabled:opacity-50 disabled:cursor-not-allowed">
                                        Lưu thay đổi
                                    </button>
                                </div>
                            </div>
                    ...
                    <div id="account-password" data-tab-panel role="tabpanel" aria-labelledby="account-password-tab"
                        class="account-tab-panel hidden card-boutique p-6">
                        <h2 class="font-heading text-lg font-semibold text-ink-900 mb-4">Đổi mật khẩu</h2>
