...
<section class="border-b border-ink-200">
    <div
        class="mx-auto grid max-w-7xl grid-cols-1 items-center gap-10 px-4 py-14 sm:px-6 lg:grid-cols-[1.1fr_1fr] lg:gap-16 lg:px-8 lg:py-20">
        <div class="animate-fade-up">
            <span class="text-serif-accent text-sm font-semibold uppercase tracking-[0.2em]">Laptop gaming &amp; hiệu
                năng cao</span>
        ...
        <div class="relative animate-fade-up" style="animation-delay:.1s">
            <div class="card-boutique overflow-hidden rounded-lg">
                <img src="./assets/images/slider/anhmoi.jpg" alt="Turbotech"
...
<section class="reveal-on-scroll py-14 md:py-20">
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div class="mb-10 max-w-xl">
            <span class="text-serif-accent text-xs font-semibold uppercase tracking-[0.2em]">Vì sao chọn
                Turbotech</span>
            <h2 class="mt-2 font-heading text-2xl font-semibold text-ink-900 sm:text-3xl">Cam kết chất lượng &amp; dịch
                vụ</h2>
        </div>
        <div
            class="grid grid-cols-1 divide-y divide-ink-200 border-y border-ink-200 sm:grid-cols-2 sm:divide-x sm:divide-y-0 lg:grid-cols-4">
    ...
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div class="mb-8 border-b border-ink-200 pb-6">
            <div class="mb-5">
                ...
                <h2 class="mt-2 font-heading text-2xl font-semibold text-ink-900 sm:text-3xl">Khám phá laptop phù hợp
                    với bạn</h2>
            </div>
            ...
            <div id="new-arrival" data-tab-panel class="tab-pane active hidden [&.active]:block!" role="tabpanel">
                <div class="grid grid-cols-2 gap-5 sm:grid-cols-3 lg:grid-cols-5">
                    ...
                    foreach ($prohome as $homeIndex => $pro) { ?>
                        <div class="card-hover group card-boutique flex flex-col overflow-hidden rounded-md<?= $homeIndex >= 10 ? ' extra-item hidden' : '' ?>"
                            data-price="<?= (int) (($pro['discount'] ?? 0) > 0 ? $pro['price'] - ($pro['price'] * $pro['discount'] / 100) : $pro['price']) ?>">
                            <div class="relative aspect-square overflow-hidden border-b border-ink-200 bg-ink-100">
                                <a class="block h-full w-full"
                                    ...
                                    <img class="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105"
                                        loading="lazy" decoding="async" src="admin/uploads/<?= e($pro['img_pro']) ?>"
                                        alt="<?= e($pro['name_pro']) ?>" />
                                </a>
                                <span
                                ...
                                <?php if ((int) $pro['stock'] <= 0) { ?>
                                    <span
                                        class="absolute bottom-2 left-2 rounded-sm bg-ink-900/80 px-2 py-1 text-[11px] font-bold text-white"><?= htmlspecialchars($pro['stock_message'] ?: 'Hết hàng') ?></span>
                                <?php } ?>
                            </div>
                            <div class="flex flex-1 flex-col p-4">
                                ...
                                <div class="mb-3 flex items-baseline gap-2">
                                    ...
                                    <?php } else { ?>
                                        ...
                                        <span
                                            class="text-sm text-ink-500 line-through"><?= number_format($pro['price']) ?>₫</span>
                                    <?php } ?>
                                </div>
                                <?php if ((int) $pro['stock'] > 0) { ?>
            ...
            <div id="bestseller" data-tab-panel class="tab-pane hidden [&.active]:block!" role="tabpanel">
                <div class="grid grid-cols-2 gap-5 sm:grid-cols-3 lg:grid-cols-5">
                    ...
                    foreach ($list_bestsp as $bestsellerIndex => $pro) { ?>
                        <div class="card-hover group card-boutique flex flex-col overflow-hidden rounded-md<?= $bestsellerIndex >= 10 ? ' extra-item hidden' : '' ?>"
                            data-price="<?= (int) (($pro['discount'] ?? 0) > 0 ? $pro['price'] - ($pro['price'] * $pro['discount'] / 100) : $pro['price']) ?>">
                            <div class="relative aspect-square overflow-hidden border-b border-ink-200 bg-ink-100">
                                <a class="block h-full w-full"
                                    href="index.php?act=prodetail&idpro=<?= e($pro['id_pro']) ?>">
                    ...
                    foreach ($list_topsp as $pro) { ?>
                        <div class="card-hover group card-boutique flex flex-col overflow-hidden rounded-md"
                            ...
                            <div class="relative aspect-square overflow-hidden border-b border-ink-200 bg-ink-100">
                                ...
                                <span
                                    class="absolute top-2 right-2 rounded-sm bg-ink-50 px-2 py-1 text-[11px] font-semibold uppercase tracking-wide text-brand-700">Nổi
                                    bật</span>
                                <?php if (!isset($pro['discount']) || $pro['discount'] <= 0) { ?>
                                <?php } else { ?>
                                    <span
...
<section class="reveal-on-scroll mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8 md:py-20">
    <div class="grid grid-cols-1 items-center gap-8 lg:grid-cols-2 lg:gap-12">
        <div class="card-boutique overflow-hidden rounded-lg lg:order-2">
            <img src="./assets/images/banner/pcgami.png" alt="PC Gaming Turbotech"
                class="h-64 w-full object-cover sm:h-80 lg:h-96" loading="lazy" decoding="async" />
        </div>
        <div class="lg:order-1">
            ...
            <div class="mt-6">
                <a class="btn-boutique inline-flex items-center justify-center gap-2 rounded-md px-6 py-3 text-sm font-semibold"
                    href="index.php?act=product&idcate=8">Mua ngay <i class="fa-solid fa-arrow-right"
...
<section class="reveal-on-scroll mx-auto max-w-7xl px-4 pb-14 sm:px-6 lg:px-8 md:pb-20">
    <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
        <a href="index.php?act=product&idcate=12"
            class="card-hover card-boutique group block overflow-hidden rounded-md">
            <img class="h-56 w-full object-cover transition-transform duration-500 group-hover:scale-105" loading="lazy"
                decoding="async" src="./assets/images/banner/lapmsi.jpg" alt="Turbotech" />
        ...
        <a href="index.php?act=product&idcate=11"
            class="card-hover card-boutique group block overflow-hidden rounded-md">
            <img class="h-56 w-full object-cover transition-transform duration-500 group-hover:scale-105" loading="lazy"
                decoding="async" src="./assets/images/banner/lenovo.jpg" alt="Turbotech" />
            <div class="flex items-center justify-between border-t border-ink-200 p-4">
...
<script>
    (function() {
        ...
        // `extra-item hidden` class added server-side past the 10th card)
